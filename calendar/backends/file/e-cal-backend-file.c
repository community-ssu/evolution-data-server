...
#include <libecal-dbus/e-cal-time-util.h>
#include <libecal-dbus/e-cal-util.h>
#include <libedata-cal-dbus/e-cal-backend-util.h>
#include <libedata-cal-dbus/e-cal-backend-sexp.h>
...
save_file_when_idle (gpointer user_data)
{
	...
	result = g_file_set_contents (priv->path, buf, -1, &error);
...
create_full_uri (ECalBackend *backend)
{
	ECalBackendFile *cbfile;
	ECalBackendFilePrivate *priv;
	...
	/* A little hacky... */
	return g_build_path ("/", master_uri, priv->file_name, NULL);
}
...
get_path (ECalBackend *backend)
{
	...
	dirname = g_filename_from_uri (topuri, NULL, NULL);
				    ...
				    GList **adds, GList **modifies, GList **deletes)
{
	...
	filename = g_strdup_printf ("%s-%s.db", priv->path, change_id);
