...
e_cal_view_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_LISTENER:
		...
		priv->listener = g_object_ref (g_value_get_pointer (value));
 ...
 **/
ECalView *
e_cal_view_new (DBusGProxy *view_proxy, ECalViewListener *listener, ECal *client)
{
	ECalView *view;

	view = g_object_new (E_TYPE_CAL_VIEW, "view", view_proxy, "listener", listener, "client", client, NULL);
